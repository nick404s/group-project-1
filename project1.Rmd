...
rev.confirmed_df <- rev(confirmed_df[,5:ncol(confirmed_df)])
# Assign NAs to 0s in the data frame
rev.confirmed_df[is.na(rev.confirmed_df)] <- 0
# create a base data frame with 4 first columns to hold result
res_df2 <- data.frame(confirmed_df[ ,1:4],stringsAsFactors = FALSE)
...
getRecentColumns <- function(df,df1)
{
  ...
  {
    for(j in 1:nrow(df))  # loop rows
    {
      # check the current and next value in the row, column. 
      # and check for NAs in latitude and longitude
      if((df[j,i + 1] == 0 && df[j,i] > 0) && !is.na(df1[j,3:4])){
        # append to the base data frame
        df1[,5:6] <- cbind(df[(i+1)],df[i])
...
res_df2 <- getRecentColumns(rev.confirmed_df, res_df2) 
