...
max.deaths.first.day.origin <- deaths_df[deaths_df[,5] == max(deaths_df[,5]), 1:4]

# Get max value of confirmed cases on the first day
max.confirmed.first.day.origin <- confirmed_df[confirmed_df[,5] == max(confirmed_df[,5]), 1:4]
...
getRecentColumns <- function(df,df1)
{
  for(i in 1:(ncol(df)-1)) # loop columns
  {
    ...
    # check the conditions and return the columns. Exclude the Olympics and Antarctica from the search
...
{
  miles_dist <- data.frame(place_df[,1:2], stringsAsFactors = FALSE) # a data frame to hold distance
  
  for (i in 1:nrow(place_df)) 
  {
    # append a distance to the data frame
    miles_dist[,3] <- cbind(round(getDistanceInMiles(origin_df[,c('Long', 'Lat')], 
                                                     place_df[i,c('Long', 'Lat')]), 2))
  }
  colnames(miles_dist)[3] <- "Distance"   # name the last column
  miles_dist[order(miles_dist$Distance),]  # return the sorted by distance data frame
}
...
sorted_countries <- sortCountriesByDistance(max.deaths.first.day.origin, last_recent_confirmed_df)
# Print the result
...
# Calculate the risk scores for each place using the last columns from the deaths and confirmed data frames
risk_score <- 100 * (om_deaths_df[,ncol(om_deaths_df)] / om_confirmed_df[,ncol(om_confirmed_df)])

# Calculate the global risk score
global_risk_score <- 100 * (sum(om_deaths_df[,ncol(om_deaths_df)]) / sum(om_confirmed_df[,ncol(om_confirmed_df)]))

# Create a data frame to hold data with the confirmed and deaths last day, and risk score 
risk_df <- cbind(om_confirmed_df[,c(1,2,ncol(om_confirmed_df))], om_deaths_df[,ncol(om_deaths_df)], risk_score)
...
sorted_by_risk_ascend <- om_risk_df[order(om_risk_df$risk_score, rev(om_risk_df$Confirmed)),]
...
cat(paste("The place with the lowest risk is:", lowest_risk_place[1,1], 
          lowest_risk_place[1,2],", the risk is:", round(lowest_risk_place[1,5],2)), sep="\n")                                                                            
...
cat(paste("The place with the highest risk score is:", highest_risk_place[1,1], 
          highest_risk_place[1,2],", the risk score is:", round(highest_risk_place[1,5],2)), sep="\n")
...
### Objective 5
```{r ob5}
# Get the country name and last day of confirmed cases data frame
last_day_confirmed <- om_confirmed_df[,c(2,ncol(om_confirmed_df))]
# Get the country name and last day of deaths cases 
last_day_deaths <- om_deaths_df[,ncol(om_deaths_df)]
# Bind them together
countries_confirmed_deaths <- cbind(last_day_confirmed,last_day_deaths)

# Use aggregate function to sum up number of confirmed and deaths cases for each country
countries_sums <- setNames(aggregate(cbind(countries_confirmed_deaths[,2],countries_confirmed_deaths[,3]) ~ countries_confirmed_deaths$Country.Region, data=countries_confirmed_deaths, FUN=sum),c("Province.State", "Sum.Confirmed", "Sum.Deaths"))

# Calculate the total sum of confirmed and death cases for each country
total_sum <- countries_sums[,2] + countries_sums[,3]

# Append it to the country sums data frame
countries_sums[,4] <- cbind(total_sum)

# Rename a column
colnames(countries_sums)[4] <- "Total.Sum"

# get 5 countries with most confirmed and deaths cases
top_5_countries <- (tail(countries_sums[order(countries_sums$Total.Sum),], 5))

# reverse rows so the most confirmed and deaths cases were on top
